...
JSON Web Tokens（JWT）作為一種認證機制，雖然具有許多優點，但也存在一些缺點：
...
1.  **Observables（可觀察對象）：** Observables 代表一個持續的數據流，可以是異步的、同步的，甚至是未來的數據。它可以發送多個值，也可以發送錯誤或完成信號。觀察者可以訂閱 Observables，並在接收到新值、錯誤或完成信號時進行相應的處理。

2.  **Operators（操作符）：** Operators 用於處理和轉換 Observables，它們提供了許多操作符來執行不同類型的操作，比如映射、篩選、合併、轉換等。Operators 可以鏈接在 Observables 上，形成一個流水線，用於處理數據流。
